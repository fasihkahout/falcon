...
function Toggle(t) {
    ...
    this.node = t.node, this.node.mt = this, this.state = t.state || !1, this.clas = t.clas || "active", this.targets = t.targets, this.scope = t.scope || null, this.isolate = t.isolate || null;
    for (var e = 0; e < this.targets.length; e++) this.targets[e].mtt ? this.targets[e].mtt.toggles.push(this.node) : new Target({
        node: this.targets[e],
        toggles: [this.node],
        state: this.state,
        clas: this.clas
    });
    if (this.node.addEventListener("click", function() {
            this.mt.changeState()
...
function Target(t) {
    return this.node = t.node, this.node.mtt = this, this.state = t.state || !1, this.clas = t.clas || "active", this.toggles = t.toggles, this.node.addEventListener("changeState", function(t) {
...
function Navbar(t, n) {
    return this.defaults = {
        ...
        anchor: {
            ...
            list: 'a[href^="#"]',
            clickCb: function() {
                window.scrollBy(0, this.anchor.getBoundingClientRect().top)
            }
        }
    }, this.node = this.qSelect(t), (this.node.navbar = this).uId = this.node.id, this.inner = this.node.querySelector(".navbar-inner"), this.wPos = null, this.state = null, this.currentMedia = null, this.currentAnchor = null, this.layout = null, this.handlers = {}, this.mParams = function() {
        var t = Navbar.map([this.defaults, n], {
            except: ["responsive"]
        });
        if (n.responsive)
            for (var e in n.responsive) t = Navbar.map([t, n.responsive[e]]);
        return t
    }.call(this), this.iParams = Navbar.merge([this.mParams, this.defaults, n]), this.dParams = Navbar.merge([this.iParams], {
        except: ["responsive"]
...
Navbar.prototype.qSelect = function(t) {
    return "string" == typeof t ? this.node.querySelectorAll(t) : t
}, Navbar.map = function(t, e) {
    e = e || {};
    for (var n = {}, s = 0; s < t.length; s++) {
        var a = t[s];
        if (a)
            for (var i in a) e.except && !e.except.indexOf(i) || (!(a[i] instanceof Object) || a[i] instanceof Node || a[i] instanceof Function ? n[i] instanceof Object || (n[i] = null) : n[i] = Navbar.map([n[i], a[i]], e))
    }
    return n
}, Navbar.merge = function(t, e) {
    e = e || {};
    for (var n = {}, s = 0; s < t.length; s++) {
        var a = t[s];
        if (a)
            for (var i in a) e.except && !e.except.indexOf(i) || (!(a[i] instanceof Object) || a[i] instanceof Node || a[i] instanceof Function ? n[i] = a[i] : n[i] = Navbar.merge([n[i], a[i]], e))
    }
    return n
}, Navbar.isUnderTop = function(t, e) {
    if (e = e || 0, t) {
        var n = t.getBoundingClientRect();
        if (n.top + e < 0 && 0 < n.bottom + e) return !0
    }
    return !1
}, Navbar.follow = function(t, e) {
    e = "string" == typeof e ? [e] : e;
    try {
        for (var n = t, s = 0; s < e.length; s++) n = n[e[s]];
        return n
    } catch (t) {
        return
    }
}, Navbar.setByPath = function(t, e, n) {
    e = "string" == typeof e ? [e] : e;
    var s, a = t;
    try {
        for (s = 0; s < e.length - 1; s++) a = a[e[s]];
        a[e[s]] = n
    } catch (t) {}
}, Navbar.enumObj = function(t, e, n) {
    for (var s in n = n || [], t) {
        var a = [].concat(n).concat([s]);
        e(s, t[s], a), t[s] instanceof Object && !(t[s] instanceof Node) && Navbar.enumObj(t[s], e, a)
    }
}, Navbar.prototype.initResponsiveness = function() {
    this.handlers.resizeHandler = this.resizeHandler.bind(this), this.handlers.mchangeHandler = this.mchangeHandler.bind(this), this.handlers.resizeHandler(), this.handlers.mchangeHandler(), window.addEventListener("resize", this.handlers.resizeHandler), this.node.addEventListener("mediachange", this.handlers.mchangeHandler)
}, Navbar.prototype.updLayout = function(t) {
    this.layout && this.node.classList.remove(this.layout), this.layout = t, this.layout && this.node.classList.add(this.layout)
}, Navbar.prototype.mchangeHandler = function() {
    this.updParams(), this.updLayout(this.dParams.layout), this.finStuck(), this.dParams.stuck && this.initStuck(), this.finAnchor(), this.dParams.anchor && this.initAnchor()
}, Navbar.prototype.updParams = function() {
    Navbar.enumObj(this.mParams, function(t, e, n) {
        var s = this.paramSelect(n);
        void 0 !== s && Navbar.setByPath(this.dParams, n, s)
    }.bind(this))
}, Navbar.prototype.paramSelect = function(t) {
    for (var e = Object.keys(this.iParams.responsive), n = e.indexOf(this.currentMedia); 0 <= n; n--) {
        if ((s = Navbar.follow(this.iParams.responsive[e[n]], t)) instanceof Object) return;
        if (void 0 !== s) return s
    }
    var s;
    return (s = Navbar.follow(this.iParams, t)) instanceof Object ? void 0 : s
}, Navbar.prototype.resizeHandler = function() {
    var t = this.currentMedia;
    for (var e in this.currentMedia = null, this.iParams.responsive) window.matchMedia("(min-width: " + e + "px)").matches && (this.currentMedia = e);
    this.currentMedia !== t && this.node.dispatchEvent(new CustomEvent("mediachange"))
}, Navbar.prototype.initStuck = function() {
    this.stuckContainer = document.querySelector(this.dParams.stuck.container), this.handlers.wPosHandler = this.wPosHandler.bind(this), this.handlers.stateHandler = this.stateHandler.bind(this), this.handlers.wPosHandler(), this.handlers.stateHandler(), window.addEventListener("scroll", this.handlers.wPosHandler), window.addEventListener("resize", this.handlers.wPosHandler), this.node.addEventListener("statechange", this.handlers.stateHandler)
}, Navbar.prototype.finStuck = function() {
    window.removeEventListener("scroll", this.handlers.wPosHandler), window.removeEventListener("resize", this.handlers.wPosHandler), this.node.removeEventListener("statechange", this.handlers.stateHandler)
}, Navbar.prototype.wPosHandler = function() {
    var t = this.state,
        e = this.stuckContainer.getBoundingClientRect(),
        n = this.node.getBoundingClientRect(),
        s = this.inner.getBoundingClientRect();
    switch (e.top >= this.dParams.stuck.offset ? this.wPos = "above-container" : n.top >= this.dParams.stuck.offset ? this.wPos = "above-wrapper" : n.bottom >= this.dParams.stuck.offset ? this.wPos = "in-wrapper" : e.bottom >= this.dParams.stuck.offset + s.height ? this.wPos = "under-wrapper" : e.bottom >= this.dParams.stuck.offset ? this.wPos = "near-container-bottom" : this.wPos = "under-container", this.wPos) {
        case "in-wrapper":
        case "under-wrapper":
            this.state = "stuck";
            break;
        case "near-container-bottom":
        case "under-container":
            this.state = "stick";
            break;
        default:
            this.state = "docked"
    }
    t !== this.state && this.node.dispatchEvent(new CustomEvent("statechange"))
}, Navbar.prototype.stateHandler = function() {
    switch (this.state) {
        case "stick":
            this.node.classList.add("stick"), this.node.classList.remove("stuck");
            break;
        case "stuck":
            this.node.classList.add("stuck"), this.node.classList.remove("stick");
            break;
        default:
            this.node.classList.remove("stick"), this.node.classList.remove("stuck")
    }
}, Navbar.prototype.initAnchor = function() {
    if (this.links = this.qSelect(this.dParams.anchor.list), this.links.length) {
        for (var t = 0; t < this.links.length; t++) {
            var e = this.links[t];
            e.anchor = document.querySelector(e.getAttribute("href")), e.anchor && (e.ref = this, e.anchClickHandler = Navbar.prototype.anchClickHandler, e.addEventListener("click", e.anchClickHandler))
        }
        this.handlers.anchPosHandler = this.anchPosHandler.bind(this), this.handlers.anchangeHandler = this.anchangeHandler.bind(this), this.handlers.anchPosHandler(), this.handlers.anchangeHandler(), window.addEventListener("scroll", this.handlers.anchPosHandler), window.addEventListener("resize", this.handlers.anchPosHandler), this.node.addEventListener("anchorchange", this.handlers.anchangeHandler)
    }
}, Navbar.prototype.finAnchor = function() {
    if (this.links && this.links.length)
        for (var t = 0; t < this.links.length; t++) {
            var e = this.links[t];
            e.removeEventListener("click", e.anchClickHandler)
        }
    window.removeEventListener("scroll", this.handlers.anchPosHandler), window.removeEventListener("resize", this.handlers.anchPosHandler), this.node.removeEventListener("statechange", this.handlers.anchangeHandler)
}, Navbar.prototype.anchPosHandler = function() {
    var t = this.currentAnchor;
    this.currentAnchor = null;
    for (var e = 0; e < this.links.length; e++) {
        var n = this.links[e];
        n.anchor && Navbar.isUnderTop(n.anchor, this.dParams.anchor.offset) && (this.currentAnchor = n)
    }
    t !== this.currentAnchor && this.node.dispatchEvent(new CustomEvent("anchorchange"))
}, Navbar.prototype.anchangeHandler = function() {
    for (var t = 0; t < this.links.length; t++) this.links[t].classList.remove("active");
    this.currentAnchor && this.currentAnchor.classList.add("active")
}, Navbar.prototype.anchClickHandler = function(t) {
    t.preventDefault();
    this.anchor.getBoundingClientRect();
    this.ref.dParams.anchor.clickCb.call(this)
};
/**
